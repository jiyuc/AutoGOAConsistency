...
if __name__ == '__main__':
    ...
    # get pre-trained go_embedding
    train_go_embeddings = FetchGOEmbedding().get_kg_embedding(pd.read_csv(train_csv)['des'].tolist())
    train_dataset = DatasetBert(train_encodings, train_labels, train_go_embeddings, binary=BINARY)  # formatting dataset
    ...
    # get pre-trained go_embedding
    dev_go_embeddings = FetchGOEmbedding().get_kg_embedding(pd.read_csv(dev_csv)['des'].tolist())
    dev_dataset = DatasetBert(dev_encodings, dev_labels, dev_go_embeddings, binary=BINARY)  # format dataset
    ...
    def compute_metrics(pred):
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(labels, preds, average='macro')  # balanced dataset
        acc = accuracy_score(labels, preds)
        with open('./output/metrics_universal.txt','a') as f:
    ...
    # begin fine-tuning
    trainer.train()
    trainer.save_model("./injections/output/5p_inject_gnn_train")
